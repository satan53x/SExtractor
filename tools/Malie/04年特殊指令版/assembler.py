...
def encode_semantic_text(text: str, encoding: str) -> bytes:
    ...
    def starts_token(pos: int) -> bool:
        if text.startswith(("$1", "$2", TOKEN_NEWLINE, TOKEN_KWAIT, TOKEN_COLOR_END, TOKEN_FONT_END, TOKEN_WAIT, "$E"), pos):
            return True
    ...
    while i < n:
        if text.startswith("$1", i):
            out.extend(MARKER_NAME1)
            i += 2
            continue
        if text.startswith("$2", i):
            out.extend(MARKER_NAME2)
            i += 2
            continue
