...
def encode_semantic_text(text: str, encoding: str) -> bytes:
    """Encode dialogue text with $1/$2 -> 07 0C 01/02 markers.

    Note: multi-chunk wait token %haato (MLS) is normally expanded to separate pool
    entries before encode. If it remains here, emit raw 0x06.
    """
    out = bytearray()
    i = 0
    n = len(text or "")
    while i < n:
        ...
        if text.startswith(TOKEN_WAIT, i):
            ...
            i += len(TOKEN_WAIT)
            continue
        # fall back to encode_text for one char / placeholders run
        # find next $ marker or end
        j = i
        while j < n and not text.startswith("$1", j) and not text.startswith("$2", j):
            j += 1
        if j > i:
