...
# ---------------------------------------------------------------------------
# Inline source-script tokens folded into dialogue text
#   $1 / $2  -> num_c 1 / num_c 2   (custom player name / sex marker)
#   %haato   -> eval FrameLayer_SendMessage("\xff\x01", 100.68)
# These are kept as source-like tokens so dialogue is not split across ops.
# ---------------------------------------------------------------------------

INLINE_NAME_TOKEN = {1: "$1", 2: "$2"}
HAATO_TOKEN = "%haato"


def _expr_is_haato(node: "ExprNode") -> bool:
    """True if expr is the heart-effect SendMessage used for %haato."""
    s = format_expr(node)
    return (
        "FrameLayer_SendMessage" in s
        and "(DOT (INT 100) (INT 68))" in s
        and ("{{01}}" in s or "\uf8f3" in s or "" in s)
    )


def _is_foldable_inline(ins: "Instr", sc: "Script") -> bool:
    if ins.mnemonic == "num_c" and ins.imm in INLINE_NAME_TOKEN:
        return True
    if (
        ins.mnemonic == "eval"
        and ins.expr_index is not None
        and 0 <= ins.expr_index < len(sc.exprs)
        and _expr_is_haato(sc.exprs[ins.expr_index])
    ):
        return True
    return False


def _inline_token_for(ins: "Instr") -> str:
    if ins.mnemonic == "num_c" and ins.imm in INLINE_NAME_TOKEN:
        return INLINE_NAME_TOKEN[ins.imm]
    if ins.mnemonic == "eval":
        return HAATO_TOKEN
    raise ValueError("not an inline token instr: %s" % ins.mnemonic)


def _is_mergeable_text(ins: "Instr") -> bool:
    # Only fold into main dialogue body (`text`). Speaker names stay on text_a/text_b.
    return ins.operand_kind == "cstr" and ins.mnemonic == "text"
...
def emit_asm(sc: Script) -> str:
    ...
    lines.append("; inline tokens: $1/$2 = player name markers (num_c); %haato = heart effect eval")
    lines.append("")
    lines.append("; ============================================================")
    ...
    while i < n:
        ...
        # Fold num_c($1/$2) and %haato eval into surrounding `text` ops.
        can_start = _is_mergeable_text(ins) or _is_foldable_inline(ins, sc)
        if can_start:
            j = i
            chunks: List[str] = []
            text_count = 0
            ...
            while j < n:
                cur = instrs[j]
                # do not swallow a label target in the middle of a merge
                if j > i and cur.pc in labels_by_off:
                    break
                if _is_foldable_inline(cur, sc):
                    chunks.append(_inline_token_for(cur))
                    fold_count += 1
                    j += 1
                    continue
                if _is_mergeable_text(cur):
                    chunks.append(cur.text or "")
                    text_count += 1
                    j += 1
                    continue
                break
            if fold_count > 0 and text_count > 0:
                lines.append('    text "%s"' % "".join(chunks))
                i = j
                continue
