...
## 0.6 文本内联标记（自定义人名 / 演出符号）

源脚本（.mls）里会出现以下内联标记；编译进 exec.dat 后会把一条台词拆成多条 code 指令。
反汇编器会把它们重新折叠回 	ext "..." 字符串，汇编时再展开，避免文本被拆散。

| 源标记 | 字节码形态 | 含义 |
|--------|------------|------|
| $1 | 
um_c 1（opcode 46 + u32 1） | 自定义主角名 |
| $2 | 
um_c 2（opcode 46 + u32 2） | 自定义性别/称呼相关标记 |
| %haato | eval → FrameLayer_SendMessage("\xff\x01", … DOT 100.68 …) | 心形演出效果（haato.png） |

说明：
- $1/$2 不是字符串里的非法字节，而是插在 	ext 之间的独立 
um_c 指令。
- %haato 也不是字符串内嵌，而是插在 	ext 之间的 eval 调用；expr 池里每个 %haato 对应一条内容相同的 expr（按出现顺序一一对应）。
- 行首旁白/独白用的 
um_a 1、
um_b 1 不折叠（不是内联人名标记）。
