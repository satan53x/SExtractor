...
# Runtime insert slots inside dialogue (07 0C NN):
#   $1 -> 07 0C 01  (custom player name 1)
#   $2 -> 07 0C 02  (custom player name 2)
#   $3.. -> 07 0C 03.. (SLG battle/item/skill/status inserts)
...
# Mid-text control bytes from Waitress.exe sub_4361B0 (IDA 0x4361B0):
#   01 RR GG BB     color set            -> $c{RRGGBB}
#   02              color restore        -> $c
#   03 NN           font size            -> $f{NN}
#   04              font restore         -> $f
#   06 XX           special 2-byte glyph -> {{06}}{{XX}}  (rare)
#   07 01 base 0A reading 00  ruby      -> $r{base|reading}
#   07 02 NN        style param          -> $s{NN}
#   07 04           soft newline         -> $e   (MLS mid-line break)
#   07 06           hard wait (mid)      -> $k   (end-of-entry 07 06 is tail=plain)
#   07 08 + "v_id"  voice id entry
#   07 09 07 06     voiced tail
#   07 0C NN        runtime insert slot  -> $N  (decimal 0-255)
#                   $1/$2 = custom player names; 04-year SLG also uses $3+
#                   for battle/item/skill/status runtime strings
#   lone 06 at chunk end (multi-entry)   -> %haato

MARKER_NAME_INSERT = bytes([0x07, 0x0C])  # + u8 slot index
MARKER_NAME1 = bytes([0x07, 0x0C, 0x01])
MARKER_NAME2 = bytes([0x07, 0x0C, 0x02])


def name_insert_bytes(slot: int) -> bytes:
    """07 0C NN runtime text insert (names, item, skill, damage, ...)."""
...
MARKER_KWAIT = bytes([0x07, 0x06])    # $k mid-text hard wait
MARKER_COLOR = 0x01
