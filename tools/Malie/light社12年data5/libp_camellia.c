/*
 * libp_camellia.c — Malie Camellia-128 加速库
 * =============================================
 * 编译 (MinGW / GCC):
 *   gcc -O2 -shared -o libp_camellia.dll libp_camellia.c
 *   gcc -O2 -shared -o libp_camellia.so  libp_camellia.c   (Linux)
 *
 * 编译 (MSVC):
 *   cl /O2 /LD libp_camellia.c
 *
 * Python 端用 ctypes 加载, 见 libp_accel.py
 */
...
/* ── 位操作 ── */
static inline uint32_t rl(uint32_t v, int n) {
    n &= 31;
    return (v << n) | (v >> (32 - n));
}
static inline uint32_t rr(uint32_t v, int n) {
    n &= 31;
    return (v >> n) | (v << (32 - n));
}
static inline uint32_t mv(uint32_t v) {
    return (rl(v,8) & 0x00FF00FFu) | (rr(v,8) & 0xFF00FF00u);
}

/* ── 单块解密 ── */
static void cam_decrypt_block(uint32_t *d, const uint32_t *kt, uint64_t offset) {
    int rb = (int)((offset >> 4) & 0x0F) + 16;
    d[0] = rl(d[0], rb); d[1] = rr(d[1], rb);
    d[2] = rl(d[2], rb); d[3] = rr(d[3], rb);
    d[0] = mv(d[0]); d[1] = mv(d[1]); d[2] = mv(d[2]); d[3] = mv(d[3]);

    int k = 0;
    d[0] ^= kt[k]; d[1] ^= kt[k+1]; d[2] ^= kt[k+2]; d[3] ^= kt[k+3];
    k += 4;
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            uint32_t t, U, D, UD;
            t = kt[k+2] ^ d[0];
            U = S3[(t>>8)&0xFF] ^ S4[t&0xFF] ^ S2[(t>>16)&0xFF] ^ S1[(t>>24)&0xFF];
            t = kt[k+3] ^ d[1];
            D = S4[(t>>8)&0xFF] ^ S1[t&0xFF] ^ S3[(t>>16)&0xFF] ^ S2[(t>>24)&0xFF];
            UD = U ^ D;
            d[2] ^= UD; d[3] ^= UD ^ rr(U, 8);
            t = kt[k] ^ d[2];
            U = S3[(t>>8)&0xFF] ^ S4[t&0xFF] ^ S2[(t>>16)&0xFF] ^ S1[(t>>24)&0xFF];
            t = kt[k+1] ^ d[3];
            D = S4[(t>>8)&0xFF] ^ S1[t&0xFF] ^ S3[(t>>16)&0xFF] ^ S2[(t>>24)&0xFF];
            UD = U ^ D;
            d[0] ^= UD; d[1] ^= UD ^ rr(U, 8);
            k += 4;
        }
        if (i < 2) {
            d[1] ^= rl(d[0] & kt[k+2], 1); d[0] ^= (d[1] | kt[k+3]);
            d[2] ^= (d[3] | kt[k+1]);      d[3] ^= rl(d[2] & kt[k], 1);
            k += 4;
        }
    }
    uint32_t tmp;
...
/* ── 单块加密 ── */
static void cam_encrypt_block(uint32_t *d, const uint32_t *kt, uint64_t offset) {
    d[0] = mv(d[0]); d[1] = mv(d[1]); d[2] = mv(d[2]); d[3] = mv(d[3]);
    int k = 51;
    d[0] ^= kt[k-3]; d[1] ^= kt[k-2]; d[2] ^= kt[k-1]; d[3] ^= kt[k];
    k -= 4;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i < 2) {
            d[1] ^= rl(d[0] & kt[k-3], 1); d[0] ^= (d[1] | kt[k-2]);
            d[2] ^= (d[3] | kt[k]);        d[3] ^= rl(d[2] & kt[k-1], 1);
            k -= 4;
        }
    }
    uint32_t tmp;
    ...
    int rb = (int)((offset >> 4) & 0x0F) + 16;
    d[0] = rr(d[0], rb); d[1] = rl(d[1], rb);
    d[2] = rr(d[2], rb); d[3] = rl(d[3], rb);
}
...
/*
 * 就地解密 data, length 字节, keytable 52个 uint32, 起始偏移 offset
 * 返回处理的块数
 */
EXPORT int camellia_decrypt(uint8_t *data, int length,
                            const uint32_t *keytable, uint64_t offset) {
    int blocks = length / 16;
    uint32_t *p = (uint32_t *)data;
    for (int i = 0; i < blocks; i++) {
        cam_decrypt_block(p, keytable, offset);
        p += 4;
        offset += 16;
    }
    return blocks;
 ...
 * 就地加密 data
 */
EXPORT int camellia_encrypt(uint8_t *data, int length,
                            const uint32_t *keytable, uint64_t offset) {
    int blocks = length / 16;
    uint32_t *p = (uint32_t *)data;
    for (int i = 0; i < blocks; i++) {
        cam_encrypt_block(p, keytable, offset);
        p += 4;
        offset += 16;
    }
    return blocks;
...
/* 版本标识, 供 Python 端检测 */
EXPORT const char* camellia_version(void) {
    return "libp_camellia 1.0 (Malie Camellia-128)";
}
