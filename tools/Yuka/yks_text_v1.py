...
def extract_dialogue(yks, encoding='cp932'):
    results = []
    # pending_name: StrOutNW设置后仅对紧随的下一个StrOut生效
    # PF(PageFeed)清除name状态 → 后续StrOut为旁白
    pending_name = None      # StrOutNW刚设置的name,等待被StrOut消费
    pending_name_id = None
    name_consumed = True     # True=name已被消费或不存在,StrOut应为旁白
    i = 0
    while i < len(yks.s1):
        si = yks.s1[i]
        if si >= len(yks.s2): i += 1; continue
        t, f1, f2, f3 = yks.s2[si]
        if t == TYPE_FUNC:
            fn = yks.s4_str(f1, encoding)
            if fn in ('StrOutNW', 'StrOutNWC'):
                # 设置待消费的角色名
                for j in range(i+1, min(i+5, len(yks.s1))):
                    sj = yks.s1[j]
                    if sj < len(yks.s2) and yks.s2[sj][0] == TYPE_STR:
                        pending_name = yks.s4_str(yks.s2[sj][2], encoding)
                        pending_name_id = sj
                        name_consumed = False
                        break
            elif fn == 'StrOut':
                for j in range(i+1, min(i+5, len(yks.s1))):
                    sj = yks.s1[j]
                    if sj < len(yks.s2) and yks.s2[sj][0] == TYPE_STR:
                        msg = yks.s4_str(yks.s2[sj][2], encoding)
                        if msg is not None:
                            if not name_consumed:
                                # StrOutNW后的第一个StrOut → 有name的对话
                                ent = {"name": pending_name, "message": msg,
                                       "message_id": sj, "name_id": pending_name_id}
                                name_consumed = True
                            else:
                                # name已消费或PF后 → 旁白
                                ent = {"name": None, "message": msg, "message_id": sj}
                            results.append(ent)
                        break
            elif fn == 'PF':
                # PageFeed → 翻页,清除name
                name_consumed = True
            elif fn in ('Select.Text1', 'Select.Text2', 'Select.Text'):
                for j in range(i+1, min(i+5, len(yks.s1))):
                    sj = yks.s1[j]
                    if sj < len(yks.s2) and yks.s2[sj][0] == TYPE_STR:
                        txt = yks.s4_str(yks.s2[sj][2], encoding)
                        if txt:
                            results.append({"name": "__select__", "message": txt, "message_id": sj})
                        break
        i += 1
    return results
