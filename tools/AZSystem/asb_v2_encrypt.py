...
def encryptFile():
	...
	crc = crc32(com, len(com), 0)
	crc_and_com = crc.to_bytes(4, byteorder='little') + com
	crc_and_com = encryptData(crc_and_com, uncomSize)
	crcComSize = len(crc_and_com)
	crc = crc_and_com[0:4]
	com = crc_and_com[4:]
	#导出
	content.clear()
	header = data[0:4] + int.to_bytes(crcComSize, 4, byteorder='little') + int.to_bytes(uncomSize, 4, byteorder='little') + crc
	content.append(header)
	content.append(com)
	write()

def encryptData(com, uncomSize):
	com = bytearray(com)
...
def crc32(data, size, key=0):
	value = ~key & 0xFFFFFFFF
	for byte in data[:size]:
		value = (Crc32Table[(value & 0xFF) ^ byte] ^ (value >> 8)) & 0xFFFFFFFF
