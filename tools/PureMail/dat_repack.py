...
def simple_repack_dat(original_dat, source_dir, output_dat):
    ...
    with open(original_dat, 'rb') as f:
        ...
        # 读取尾部信息
        f.seek(file_size - 12)
        packed_size = struct.unpack('<I', f.read(4))[0] ^ 0xF0F0F0F0
        unpacked_size = struct.unpack('<I', f.read(4))[0] ^ 0xF0F0F0F0
    ...
    entry_count = len(original_unpacked_index) // 0x50
    ...
    for i in range(entry_count):
        entry_offset = i * 0x50
        entry_data = new_index[entry_offset:entry_offset + 0x50]
        
        # 解析条目
        flags = struct.unpack('<I', entry_data[0:4])[0]
        name_bytes = entry_data[4:0x44]
        ...
        if source_file.exists():
            ...
            struct.pack_into('<I', new_index, entry_offset + 0x44, file_offset)  # offset
            struct.pack_into('<I', new_index, entry_offset + 0x48, file_size)  # size
            struct.pack_into('<I', new_index, entry_offset + 0x4C, file_size)  # unpacked_size
            ...
            struct.pack_into('<I', new_index, entry_offset + 0x44, current_offset)  # offset
            struct.pack_into('<I', new_index, entry_offset + 0x48, 0)  # size = 0
            struct.pack_into('<I', new_index, entry_offset + 0x4C, 0)  # unpacked_size = 0
    ...
    with open(output_dat, 'wb') as f:
        ...
        f.write(struct.pack('<I', packed_size ^ 0xF0F0F0F0))
        f.write(struct.pack('<I', unpacked_size ^ 0xF0F0F0F0))
        f.write(struct.pack('<I', 0))  # 保留字段
    
    output_size = len(new_data) + len(compressed_index) + 12
