...
class ObjProcessor:
    ...
    def extract_strings_from_obj(self, obj_path):
        ...
        try:
            ...
            # 读取第一个索引条目（其他字节区，不提取）
            first_addr = struct.unpack('<H', data[0:2])[0]
            first_length = struct.unpack('<H', data[2:4])[0]
            
            # 读取第二个索引条目来确定索引区结束位置
            string_start = struct.unpack('<H', data[4:6])[0]
            
            # 计算索引条目数量
            index_count = string_start // 4
    ...
    def rebuild_obj_file(self, txt_path, original_obj_path, output_obj_path):
        ...
        try:
            ...
            # 读取原始文件的第一个索引条目（字符串区结尾地址信息）
            first_addr = struct.unpack('<H', original_data[0:2])[0]
            first_length = struct.unpack('<H', original_data[2:4])[0]
            ...
            # 第一个索引：其他字节区（地址指向字符串区结尾，即其他字节区开始位置）
            other_bytes_entry = struct.pack('<HH', other_bytes_start, first_length)
            index_data += other_bytes_entry
            
            # 其他索引：字符串区
            for i, (addr, length) in enumerate(new_indices):
                index_entry = struct.pack('<HH', addr, length)
